	...
	if (elements == "all" || elements == "recommended") {
		...
		elements.forEach(function (element) {
			...
			switch (element) {
				...
				case "field-mob": //KS:
					$(
						"[type='text'], [type='date'], [type='email'], [type='number'], [type='password'], [type='tel'], [type='time'], [type='textarea']"
					).addClass("field-mob");
					break;
				...
				case "field-text-align-left":
					$(
						"[type='text'], [type='date'], [type='email'], [type='number'], [type='password'], [type='tel'], [type='time']"
				...
				case "field-text-align-right":
					$(
						"[data-type='text'], [data-type='date'], [data-type='email'], [data-type='number'], [data-type='password'], [data-type='tel'], [data-type='time']"
					).addClass("text-align-right");
					break;
				...
				case "search-empty-search":
					...
					break;
				case "reset-search":
					listenerFunctions["reset-search"]();
					break;
				default:
					validStyle = false;
			}
...
function applyNewStyle() {
	...
	var hasDefaultsInArguments =
		typeof arguments[0] !== "undefined" && Array.isArray(arguments[0]);
	if (hasDefaultsInArguments) {
		if (debugStyle)
			console.debug(
				"@applyNewStyle() since this was passed an array, will call defaultNewStyle() to add classes to relevent objects before continuing"
			);
	...
	if (debugStyle)
		console.debug(
			"@applyNewStyle() the list classes used as the selector and the name of the function are: " +
			JSON.stringify(elementsToUpdate)
		);
	elementsToUpdate.forEach(function (item) {
		var elements = $(item[0]);
		if (elements.length > 0) {
			//KS: skip if none selected - improve performance
			if (item.length == 1) {
				//KS: presumed the selector is the function name (with first '.' removed)
	...
	$(formName()).trigger("_style_styleApplied", [
		elementsToUpdate,
		hasDefaultsInArguments ? arguments[0] : false,
	]);
}
...
var updateStyleFunctions = {
	...
	"warning-notice": function (element) {
		...
		el.append(
			'<span class="warning-notice-icon"><span class="warning-notice-icon-a"></span><span class="warning-notice-icon-b"></span><span class="warning-notice-icon-c"></span></span>'
		);
	},
	"info-notice": function (element) {
		var el = element.not(":has(.info-notice-icon-a)");
		el.append(
			'<span class="info-notice-icon"><span class="info-notice-icon-a"></span><span class="info-notice-icon-b"></span><span class="info-notice-icon-c"></span></span>'
		);
	},
	...
	"chk-gov": function (element) {
		...
		$(".dform_widget_type_checkbox :not(span) > input").each(function() {
	...
	"file-gov": function (element) {
		...
		el.after(
			'<span class="file-gov-icon"><span class="file-gov-icon-a"></span><span class="file-gov-icon-b"></span><label class="file-gov-text">Select Files...</label></span>'
		);
	...
	"detail-gov": function (element) {
		...
		element.each(function () {
			$(this)
				.children(":not(button)")
				.wrapAll('<div class="detail-block"></div>');
		});
	},
	"required-notice": function (element) {
		...
		updateStyle(
			element.addClass("info-notice width-fit-content"),
			"info-notice"
		);
		element.find("p, li").addClass(classStyle);
	},
	"file-limit": function (element) {
		//KS: need to test what happens if over limit when it's applied
		...
		if (hasClass) {
			...
			element
				.find(".dform_filenames")
				.off("DOMNodeInserted DOMNodeRemoved")
				.on("DOMNodeInserted DOMNodeRemoved", function (event) {
					var current = $(this).children("span").length;
					if (event.type == "DOMNodeInserted") {
						//KS: adding a file
						if (current >= number) {
							//KS: Can't add more
							$(this).parent().find("input").addClass("display-none");
							...
							$(this)
								.parent()
								.find(".file-gov-text")
								.text("Select up to " + (number - current) + " more");
							//KS: trigger: '_style_fileUploaded, [currentFileNumber, maxFiles, slotsFree]'
							$(formName()).trigger("_style_fileUploaded", [
								current,
								number,
								number - current,
							]);
						...
						if (current - 1 == 0) {
							...
							$(this)
								.parent()
								.find(".file-gov-text")
								.text("Select up to " + number + " files");
						} else {
							//KS: at least one file is uploaded - display number left
							$(this)
								.parent()
								.find(".file-gov-text")
								.text("Select up to " + (number - (current - 1)) + " more");
						}
						//KS: trigger: '_style_fileUploaded, [currentFileNumber, maxFiles, slotsFree]'
						$(formName()).trigger("_style_fileUploaded", [
							0,
			...
			if (debugStyle)
				console.debug(
					"A file limit couldn't be applied to an element because it didn't have a file-limit-[number] style "
				);
		}
	},
	...
	"rad-margin": function (element) {
		...
		if (hasClass) {
			...
			if (Number.isInteger(number) && number >= 0 && number <= 100) {
				//KS: since it is %, unlikely to go over
				marginArrange(
					element.find("legend"),
					"rad-margin-" + number + "-legend"
				);
			} else {
				if (debugStyle)
					console.debug(
						hasClass + "is not a valid rad-margin, try rad-margin-50"
					);
			}
		} else {
			if (debugStyle)
				console.debug(
					"Could not add rad-margin to element. Try adding the class rad-margin-# (e.g. rad-margin-50) first"
				);
		}
	},
	"mchk-margin": function (element) {
		...
		if (hasClass) {
			...
			if (Number.isInteger(number) && number >= 0 && number <= 100) {
				...
				marginArrange(
					element.find("legend"),
					"mchk-margin-" + number + "-legend"
				);
			} else {
				if (debugStyle)
					console.debug(
						hasClass + "is not a valid mchk-margin, try mchk-margin-50"
					);
			}
		} else {
			if (debugStyle)
				console.debug(
					"Could not add mchk-margin to element. Try adding the class mchk-margin-# (e.g. mchk-margin-50) first"
				);
		}
	},
};
...
function individualApplyStyle(element, specificVal) {
	...
	if (specificVal !== null) {
		//KS: when provided with a style name
		if (updateStyleFunctions[specificVal] != undefined) {
			//KS: update style when valid
			updateStyleFunctions[specificVal](element);
			//KS: trigger: '_style_elementUpdated, [element, source, functionUsed, wasSpecified]'
			$(formName()).trigger("_style_elementUpdated", [
				element,
				specificVal,
				true,
			]);
		} else {
			//KS: can't find style - tell them so within collapsable group
			if (debugStyle)
				console.debug(
					"Style not updated - style name was " +
					specificVal +
					" and element was:"
				);
			...
			if (debugStyle)
				console.debug(
					'Try a valid name from "updateStyleFunctions" or try it without a name for default functionility'
				);
		}
	} else {
		//KS: DEFAULTS when no style name is provided, attempt to apply one based on class
		...
		if (!hasAddedStyle) {
			//KS: just a log to update them that something went wrong
			if (debugStyle)
				console.debug(
					"No name provided, and failed class checks. Element was:"
				);
			if (debugStyle) console.debug(element);
			if (debugStyle)
				console.debug(
					'Try a valid name from "updateStyleFunctions" as the second param to specify type of update'
				);
		}
	}
}
...
var listenerFunctions = {
	...
	noResultsFound: function () {
		$(formName()).on("_KDF_search", function (
			event,
			kdf,
			response,
			type,
			...
			//OB: show widget legend if it has class sr-only-legend
			if ($('#dform_widget_search_' + name).hasClass('sr-only-legend')) {
				$('#dform_widget_search_' + name).addClass('js-toggle-legend').toggleClass('sr-only-legend');
			}

			//KS: call noResultsFound with 'this' set to the search element that triggered the event
			noResultsFound.call($('[name="' + name + '_id"]'), type, name);
		});
		//KS: trigger: '_style_listenerAdded, [listenerName]'
	...
	selectResult: function () {
		...
		$(formName()).on("_KDF_search", function (
			...
			selectResult.call($('[name="' + name + '_id"]'), type, name);
		});
		//KS: trigger: '_style_listenerAdded, [listenerName]'
	...
	"search-empty-search": function () {
		...
		$(formName())
			.find('button[data-type="searchwidget"]')
			.off("click")
			.on("click", function (e) {
				KDF.hideMessages();
				var valid = 0;
				$(this)
					.closest(".searchwidget")
					.find(".dform_widget_searchfield:visible :input")
					.each(function () {
						if ($(this).val() !== "") {
							valid += 1;
						}
	...
	"reset-search": function () {
		$(document).on('click', '.dform_widget_search_closeresults', function () {
			// hide widget legend if it has class js-toggle-legend
			if ($(this).parent().parent().parent().hasClass('js-toggle-legend')) {
				$(this).parent().parent().parent().toggleClass('sr-only-legend');
			}
			// clear any validation errors
			$(this).parent().parent().removeClass('dform_widgeterror');
			$(this).parent().siblings('.dform_validationMessage').hide();
...
function txtaLength() {
	...
	$(formName()).trigger("_style_lengthChanged", [
		$(this),
		message,
		currentLength >= maxLength ? true : false,
		maxLength,
		currentLength,
	]);
}
...
function detailToggle() {
	...
	//KS: update to use value of attributes like closedChar='Ã¢â€“Âº' openedChar='Ã¢â€“Â¼' - ensures uses defined chars
	if ($(this).text().indexOf(prefix.closed) >= 0) {
		$(this).text(
			$(this).text().replace(new RegExp(prefix.closed, "g"), prefix.opened)
		);
		$(this).siblings(".detail-block").addClass("detail-block-visible");
		open = true;
	} else {
		$(this).text(
			$(this).text().replace(new RegExp(prefix.opened, "g"), prefix.closed)
		);
	...
	$(formName()).trigger("_style_detailToggled", [
		$(this),
		open,
		prefix.opened,
		prefix.closed,
...
function regexSearch(regex, selector) {
	if (selector === undefined) {
		selector =
			".search-gov input:text, .apply-regex, #dform_widget_txt_postcode";
...
function paramElementChange(possibleToChange) {
	...
	if (params.wss == "true") {
		$.each(params, function (key, value) {
			if (
				possibleToChange.includes(key) &&
				!defaultParams.includes(defaultParams)
			) {
...
function simpleColorCheck(origBgColor, fgIfWhite, altFg) {
	...
	$(formName()).trigger("_style_simpleColorCheck", [
		origBgColor,
		bgColor,
		choosenFg,
		fgIfWhite,
...
function requiredColorCheck(jQueryObject, defaultColor, altColor) {
	...
	var color = simpleColorCheck(
		jQueryObject.css("background-color"),
		defaultColor,
		altColor
	);

	//KS: trigger: '_style_colorChecked, [element, defaultColor, altColor]'
	$(formName()).trigger("_style_colorChecked", [
		jQueryObject,
		defaultColor,
		altColor,
	]);
...
function highlightRequired() {
	...
	var eligible = [
		...
		[
			$(
				".txt-gov,.dt-gov,.eml-gov,.num-gov,.pas-gov,.tel-gov,.time-gov,.txta-gov"
			),
			function (val) {
	...
	var reqFun = {
		isEligible: function (element) {
			return element.find('abbr[title="required"]').length < 1;
		},
		apply: function (element) {
			return (
				'<abbr title="required" style="color: ' +
				requiredColorCheck(element) +
				';"> *</abbr>'
			);
		},
	};
...
function getFieldsLabels(isPosLeft) {
	...
	if (isPosLeft && isPosLeft != "above") {
		if (debugStyle)
			console.debug(
				"@getFieldsLabels() a selector for elements with a label to the left is being generated. The elements being considered are: " +
		...
		for (var i = 0; i < elements.length; i++) {
			for (var j = 0; j < columns.length; j++) {
				selector +=
					", " +
					elements[i] +
					":not(.dform_widget_searchfield) > div:first-child" +
					columns[j] +
					" label";
		...
		if (debugStyle)
			console.debug(
				"@getFieldsLabels() a selector for elements with a label above is being generated. The elements being considered are: " +
				JSON.stringify(elements)
			);
		...
		for (var i = 0; i < elements.length; i++) {
			selector +=
				", " +
				elements[i] +
				" > div:first-child:not(.one,.two,.three,.four,.five,.six,.seven,.eight,.nine,.ten,.eleven,.twelve) label";
		}
	}
